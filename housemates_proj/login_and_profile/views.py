import re
from .models import User, Item, HouseMembership, House, BalanceDue, HouseNotifications
...
def profile(request):
    ...
    memberships = this_user.users_house.all()
    context = {
        # add notifications?
        'user': this_user,
...
def create_house(request):
    ...
    membership = HouseMembership.objects.create(
        house=new_house, pending_invite=False)
    this_user.users_house.add(membership)
    HouseNotifications.objects.create(
...
def add_housemate(request):
    ...
    HouseNotifications.objects.create(
        sender=sender, action="INVITED", house=house)
    return redirect('/profile/main_house/')
